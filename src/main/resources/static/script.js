...
async function fetchEmployeeTask() {
    try {
        ...
        // Send a fetch request to the backend with the email as a parameter
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch employee tasks: ${response.status} ${response.statusText}`);
        }

        // Parse the JSON response
        const tasks = await response.json();

        // Log the tasks or process them further
        console.log('Employee Tasks:', tasks);

        // You can update the DOM or handle the tasks here
        ...
        if (taskContainer) {
            ...
            tasks.forEach((task) => {
                ...
                taskElement.textContent = task.description; // Assuming tasks have a 'description' field
...
async function displayEmployeeTasks(employeeId) {
    ...
    try {
        const response = await fetch(`/api/employees/tasks?employeeId=${employeeId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        // Tratarea răspunsurilor cu cod 204 No Content
        if (response.status === 204) {
